...
parser.add_argument("--finetune_file", help="Json File to extract info", type=str)
...
Example:
    Linux:
        python3 generate_json.py --sql_file=../db.sqlite3 --finetune_file=../finetuning/ --output=../optuna/optuna_metadata.json
    Windows:
...
def get_query():
    query = """
        SELECT t.trial_id, t.number, s.study_name, tv.value, tua.key, tua.value_json 
        FROM trials as t
        JOIN trial_values as tv
        ON tv.trial_id = t.trial_id
        JOIN trial_user_attributes as tua
        ON tua.trial_id = t.trial_id
        JOIN studies as s
        ON t.study_id = s.study_id
        WHERE tua.key='train_accuracy'
        ORDER BY tua.value_json DESC
    """
    return query
